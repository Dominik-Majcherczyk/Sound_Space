...
export default function Song({ song, setCurrentId, uniqueKey }) {
  ...
  return (
    <div key={uniqueKey}>
      <div className="overflow-hidden shadow-lg rounded-lg w-60 md:w-80 ">
        <img
          alt="blog photo"
        ...
        <div className="bg-white dark:bg-gray-800 w-full p-4">
          ...
          <div className="text-gray-400 dark:text-gray-300 font-light text-md">
            <TextTruncate
              line={2}
              element="p"
              truncateText="…"
          ...
          <div className="flex gap-8 justify-starts items-center mt-4">
            ...
            {(user?.result?.googleId === song?.author ||
              user?.result?.name === song?.author) && (
              <button
                type="button"
          ...
          <p className="text-sm ">
            Created by: <span className="font-bold">{song.author}</span>
          </p>
        </div>
      </div>
    </div>
